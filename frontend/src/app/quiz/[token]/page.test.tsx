...
vi.mock("@/lib/api", async (importOriginal) => {
  ...
  return {
    ...original,
    publicQuiz: { state: vi.fn(), next: vi.fn(), answer: vi.fn() },
  };
});
...
describe("QuizPage", () => {
  ...
  it("shows the intro with the rules, then serves a question", async () => {
    ...
    // options rendered in served order
    const buttons = screen.getAllByRole("button");
    expect(buttons[0]).toHaveTextContent("Any use of threads");
  });

  it("answers and advances to the done state", async () => {
    mocked.next
