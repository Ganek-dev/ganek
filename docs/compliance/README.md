# Vetd compliance pack (GDPR)

Documents for companies that self-host Vetd and process candidates in (or
from) the EU/EEA and UK. If that is you, **you are the data controller**
for your candidates' data — Vetd is software you run, and this pack exists
...
> they describe accurately what Vetd does with personal data and map that
...
never of software alone. Vetd ships as **GDPR-ready**:
...
- **DSAR export**: a per-candidate JSON bundle of everything Vetd holds
...
- **No automated decisions**: scores and integrity flags inform humans;
  Vetd never auto-rejects, auto-advances, or auto-filters anyone.
...
| [Records of processing template](records-of-processing-template.md) | Art. 30 record pre-filled with Vetd's processing activities | Keep current from day one — the small-company derogation does not apply to recruitment |
| [Breach runbook](breach-runbook.md) | Detect → assess → notify steps with Vetd-specific containment levers, plus a breach register template | When something goes wrong (read it once before that) |
...
| [AI Act statement](ai-act-statement.md) | Why Vetd's deterministic scoring is not an AI system, and the guardrail that keeps it that way | Procurement/vendor-assessment questionnaires |

## If you change Vetd

These documents describe **upstream Vetd as shipped**. Adding analytics or
