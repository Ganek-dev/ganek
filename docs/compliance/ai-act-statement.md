...
> Vetd as shipped; not legal advice — see the [pack README](README.md).

**Vetd does not contain an AI system.** Employment and recruitment tools
...
**1. Vetd's scoring is not an "AI system" under Art. 3(1).** The quiz
...
natural persons; Vetd's engine is squarely that. (The guidelines are
...
AI systems to that date. Vetd doesn't rely on this — ground 1 is the
...
Vetd: scores and integrity flags are displayed to reviewers who always see
...
without meaningful human involvement; Vetd's design keeps humans doing the
...
Upstream Vetd's position: **no such feature ships without redoing the AI
